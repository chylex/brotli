...
static void StoreSymbolWithMultiContext(BlockEncoder* self, size_t symbol,
    size_t prev, size_t prev2, const uint32_t* context_map, const ContextType* context_modes,
    size_t* storage_ix, uint8_t* storage, const size_t context_bits) {
  if (self->block_len_ == 0) {
    size_t block_ix = ++self->block_ix_;
    uint32_t block_len = self->block_lengths_[block_ix];
    uint8_t block_type = self->block_types_[block_ix];
  ...
  --self->block_len_;
  {
    const size_t block_type = self->block_types_[self->block_ix_];
    const ContextLut context_lut = BROTLI_CONTEXT_LUT(context_modes[block_type]);
    const size_t context = BROTLI_CONTEXT(prev, prev2, context_lut);
...
void BrotliStoreMetaBlockMultiLiteralContextMode(MemoryManager* m,
    const uint8_t* input, size_t start_pos, size_t length, size_t mask,
    uint8_t prev_byte, uint8_t prev_byte2, BROTLI_BOOL is_last,
  ...
  BrotliWriteBits(
      ...
      storage_ix, storage);
  for (i = 0; i < mb->literal_split.num_types; ++i) {
    BrotliWriteBits(2, mb->literal_context_modes[i], storage_ix, storage);
  }
  ...
  for (i = 0; i < n_commands; ++i) {
    ...
    if (mb->literal_context_map_size == 0) {
      ...
      for (j = cmd.insert_len_; j != 0; --j) {
        uint8_t literal = input[pos & mask];
        StoreSymbolWithMultiContext(&literal_enc, literal, prev_byte, prev_byte2,
            mb->literal_context_map, mb->literal_context_modes, storage_ix, storage,
            BROTLI_LITERAL_CONTEXT_BITS);
        prev_byte2 = prev_byte;
        prev_byte = literal;
        ++pos;
